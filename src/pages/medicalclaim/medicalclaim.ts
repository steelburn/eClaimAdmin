...
export class MedicalclaimPage {
  ...
  GetDataforEdit() {
    this.http
      .get(getURL("table", "main_claim_request", [`CLAIM_REQUEST_GUID=${this.claimRequestGUID}`]))
      .map(res => res.json())
      .subscribe(data => {
  ...
  saveIm() {
    let uploadImage = UploadImage(this.http, this.uploadFileName, this.Medicalform.get('avatar'))
