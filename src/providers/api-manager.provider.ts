...
export class ApiManagerProvider {
  ...
  LoadMainClaim(claimReqGUID:string) {
    let totalAmount: number;
    return new Promise((resolve, reject) => {
      this.getApiModel('view_claim_request', 'filter=CLAIM_REQUEST_GUID=' + claimReqGUID).subscribe(res => {
        let claimDetailsData:any[] = res['resource'];
        claimDetailsData.forEach(element => {
    ...
    return this.http
      ...
      .map((response) => {
       ...
       // console.log(JSON.stringify(result));
        ...
        let resData:any[]=result.resource;
       // console.log(JSON.stringify(resData));
       // result.resource.forEach((bank) => {

        resData.forEach((bank) => {
         // alert(bank);
  ...
  //   queryHeaders.append('X-Dreamfactory-API-Key', constants.DREAMFACTORY_API_KEY);
  //   return  new Promise((resolve, reject) => {
  //      this.http.get(this.getModelUrl(endPoint, args), { headers: queryHeaders })
  ...
  //   queryHeaders.append('X-Dreamfactory-API-Key', constants.DREAMFACTORY_API_KEY);
  //   return new Promise((resolve, reject) => {
  //     this.http.get(url, { headers: queryHeaders })
  //       .map(res => {
  //         resolve(res.json())
  //       })
  //   })
  ...
  //   queryHeaders.append('Content-Type', 'application/json');
  //   queryHeaders.append('X-Dreamfactory-API-Key', constants.DREAMFACTORY_API_KEY);
  //   return  this.http.get(url, { headers: queryHeaders })
  ...
  //   }
  // }
  // getApiModel2(endPoint: string, args?: string) {
  //   let url = this.getModelUrl(endPoint, args);
  //   var queryHeaders = new Headers();
  //   queryHeaders.append('Content-Type', 'application/json');
  //   queryHeaders.append('X-Dreamfactory-API-Key', constants.DREAMFACTORY_API_KEY);

  //   let response = this.http.get(url, { headers: queryHeaders }).toPromise();
  //   return response.json().data as ClaimReqMain_Model[];
  //   // .map(res => res.json())
  //   // .subscribe(data => {
  //   //   resolve(data);
  //   // })   
  ...
  //   	queryHeaders.append('Content-Type', 'application/json');		
  //   	queryHeaders.append('X-Dreamfactory-API-Key', constants.DREAMFACTORY_API_KEY);
  // 	return this.http.get(this.getUrl(endPoint, args), {headers: queryHeaders})
  // 		.map((response) =>   // 		{
