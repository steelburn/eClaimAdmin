...
export class ApiManagerProvider {
      ...
      .subscribe(data => {
        ...
        if (email_details.length > 0) {
          ...
          if (ApproverStatus == "Rejected") {
            strSubjectApplier = "Your Claim is Rejected"; Description = Remarks;
          }
          else {
  ...
  isClaimExpired(formValues: any) {
    ...
    let travelMonth = myDate.getMonth();
    let currentMonth = new Date().getMonth();
    let travelDate = myDate.getDate();
 
    if (travelMonth < currentMonth && travelDate > 7) {
