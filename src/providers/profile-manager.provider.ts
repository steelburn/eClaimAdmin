...
export class ProfileManagerProvider {
  userGUID:string; TenantGUID:string; previousLevel: number; previousAssignedTo: string; level:number;
  mainClaimReq: MainClaimRequestModel; claimRequestGUID:string; isRemarksAccepted:boolean;
  ...
  profileLevel:number; assignedTo:string; stage:string;
  ...
  getMainClaimReqInfo(claimRef: ClaimWorkFlowHistoryModel, level:number, claimRequestGUID:string, isRemarksAccepted:boolean) {
    this.level = level;
  ...
  GetDirectManagerByManagerGUID() {
    this.api.getApiModel('user_info', 'filter=USER_GUID=' + this.assignedTo).subscribe(res => {
      let managerInfo:any[] = res["resource"]
      managerInfo.forEach(userElm => {
  ...
  SaveWorkFlow(claimRef: ClaimWorkFlowHistoryModel) {
    ...
    this.api.postData('claim_work_flow_history', claimRef.toJson(true)).subscribe((response) => {
      ...
      this.mainClaimReq.UPDATE_TS = new Date().toString();
      if (this.level === -1)
  ...
  ProcessProfileMng(remarks:any, approverGUID:string, level:number, claimRequestGUID:string, isRemarksAccepted:boolean) {
    debugger;
    ...
    claimHistoryRef.CREATION_TS = new Date().toString();
    claimHistoryRef.UPDATE_TS = new Date().toString();
  ...
  //  var condElement:any[]=[];
    ...
    let levels:any[] = profileJSON.profile.levels.level
    let nextLevel;
    levels.forEach(element => {
      if (element['-id'] == this.level) {
        let temp:any[]=element['conditions']['condition'];
    ...
    if (this.level === -1 || this.level === 0) {
      ...
      this.getInfoLevels(levels, this.level)
      // levels.forEach(element => {
      //   if (element['-id'] == this.level) {
  ...
  getInfoLevels(levels:any[], level:number) {
    levels.forEach(element => {
      if (element['-id'] == level) {
  ...
  saveClaimRef(month:number, year:number) {
